...
class CarouselSelection {
    ...
    setupQuantityControls() {
        ...
        decreaseBtns.forEach(btn => {
            btn.addEventListener('click', this.handleQuantityDecrease.bind(this));
        });
        
        increaseBtns.forEach(btn => {
            btn.addEventListener('click', this.handleQuantityIncrease.bind(this));
        });
        ...
        this.items.forEach(item => {
            ...
            if (quantityControl) {
                // Add click event to the item itself (excluding the quantity control)
                item.addEventListener('click', (event) => {
                    // Ignore clicks on the quantity control itself
                    if (!event.target.closest('.quantity-control')) {
                        const input = item.querySelector('.quantity-input');
                        if (input && parseInt(input.value) === 0) {
                            // Set quantity to 1 on first click
                            input.value = 1;
