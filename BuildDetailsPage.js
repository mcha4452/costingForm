...
class BuildDetailsPage {
     ...
     */
    reset() {
        console.log('BuildDetailsPage: Performing full reset of all components');
        
        // Reset all components
        if (this.windowsSelection) this.windowsSelection.reset();
        if (this.doorsSelection) this.doorsSelection.reset();
        if (this.skylightsSelection) {
            console.log('Resetting skylights selection component');
            this.skylightsSelection.reset();
        } else {
            console.log('No skylights selection component found during reset');
        }
        ...
        // Reset and properly reinitialize the skylights container
        this.resetSkylights();
        
        // Re-initialize skylights carousel after reset
        const skylightsContainer = this.pageElement.querySelector('#skylightsContainer');
        if (skylightsContainer) {
            console.log('Re-initializing skylights carousel after reset');
            
            // Create a new CarouselSelection instance to replace the old one
            this.skylightsSelection = new CarouselSelection(
                skylightsContainer,
                {},
                this.handleSkylightsSelection.bind(this)
            ...
            // Manually ensure all skylights are properly set up
            const skylightItems = skylightsContainer.querySelectorAll('.carousel-item');
            skylightItems.forEach(item => {
                const quantityControl = item.querySelector('.quantity-control');
                if (quantityControl) {
                    // Ensure the quantity control starts hidden
                    quantityControl.style.display = 'none';
                    
                    // Get quantity input and ensure it's reset
            ...
            console.log('Skylights carousel re-initialized with', skylightItems.length, 'items');
        } else {
            console.warn('Could not find skylights container during reset re-initialization');
        }
